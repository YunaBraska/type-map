...
import berlin.yuna.typemap.model.TypeI;
...
public class JsonDecoder {
    ...
    /**
     * Determines the type of the given JSON string and converts it into a corresponding {@link TypeI} instance.
     * This method utilizes the {@link #jsonOf(String)} method to parse the JSON string and then checks the resulting
     * object's type to appropriately wrap it within a {@link TypeI}. If the result is already an instance of
     * {@link TypeI}, it is returned directly. Otherwise, the result is wrapped in a {@link TypeList} to maintain
     ...
     * @return A {@link TypeI} instance representing the structured data of the JSON input. If the input is
     *         a JSON object or array, the returned {@link TypeI} will be a {@link TypeMap} or {@link TypeList},
    ...
    public static TypeI<?> jsonTypeOf(final String jsonOrXml) {
        final Object result = jsonOrXml != null && jsonOrXml.startsWith("<")? XmlDecoder.xmlTypeOf(jsonOrXml) : jsonOf(jsonOrXml);
        return result instanceof TypeI ? (TypeI<?>) result : new TypeList().addReturn(result);
    }
